...
of the application startup. Compilation is started by executing \code{npm
  run-script electronbuild} on the comgmand line, the resulting artifacts will be
...
\begin{figure}
\begin{subfloat}[The bundled Electron app. Later versions will
  add some kind of native menu.]{\includegraphics[max width=0.32\linewidth]{images/electronapp}}\end{subfloat}
\begin{subfloat}[Embedded web version, with SingleView plugin and
  without Converter, Bookmarks and Paint.]{\includegraphics[max width=0.32\linewidth]{images/embedded}}\label{embedded}\end{subfloat}
\begin{subfloat}[Implementation running on a Huawei P10
  mobile phone with the Chrome browser. Opening a BTF is done through clicking the
  upload area.]{\includegraphics[max width=0.32\linewidth]{images/rtimobile}}\label{rtimobile}\end{subfloat}
\caption[Target Comparison]{Target Comparison}
